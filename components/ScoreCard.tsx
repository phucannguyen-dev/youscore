...
export const ScoreCard: React.FC<ScoreCardProps> = ({ 
  ...
  return (
    <div 
      ...
      className={`bg-white dark:bg-slate-900 rounded-xl p-4 shadow-sm border transition-all hover:shadow-md flex items-center justify-between group break-inside-avoid ${
    ...
    >
      ...
      {isSelectMode && (
        <div className="mr-4 text-indigo-600 dark:text-indigo-400">
          {isSelected ? <CheckSquare className="w-5 h-5" /> : <Square className="w-5 h-5 text-slate-300 dark:text-slate-600" />}
        </div>
      )}
      ...
      <div className="flex-1">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            <div className="relative group/badge">
                <span className={`px-2 py-0.5 text-xs font-semibold rounded-full uppercase tracking-wide flex items-center gap-1 cursor-pointer ${
                entry.examType === ExamType.FINAL 
                    ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300' 
                    : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700'
                }`}>
                {entry.examType}
                {!isSelectMode && <ChevronDown className="w-3 h-3 opacity-50" />}
                </span>
                {!isSelectMode && (
                  <select
            ...
            {showDate && (
              <div className="relative group/date">
                  <span className="text-xs text-slate-400 dark:text-slate-500 flex items-center group-hover/date:text-indigo-500 dark:group-hover/date:text-indigo-400 transition-colors cursor-pointer">
        ...
        <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">{entry.subject}</h3>
        <p className="text-sm text-slate-500 dark:text-slate-400 italic truncate max-w-[200px] print:hidden">"{entry.originalText}"</p>
      </div>
      ...
      <div className="flex items-center gap-4">
        {isEditing ? (
            <div className="flex items-center gap-1 px-2 py-1 rounded-lg border bg-white dark:bg-slate-800 border-indigo-200 dark:border-indigo-800 shadow-sm animate-in fade-in zoom-in-95" onClick={e => e.stopPropagation()}>
                <input 
                    ...
                    className="w-12 text-right bg-transparent border-b border-slate-200 dark:border-slate-700 focus:border-indigo-500 outline-none text-sm font-bold text-slate-900 dark:text-white p-0"
                    step="any"
                />
                ...
                <input 
                    ...
                    onKeyDown={handleKeyDown}
                    className="w-12 bg-transparent border-b border-slate-200 dark:border-slate-700 focus:border-indigo-500 outline-none text-sm text-slate-500 p-0"
                    step="any"
                />
            ...
            <div 
                ...
                className={`relative flex flex-col items-end px-3 py-1 rounded-lg border ${colorClass} transition-all group/score select-none ${isSelectMode ? '' : 'cursor-pointer hover:brightness-95'}`}
        ...
        {!isSelectMode && (
          <button 
            ...
            className="p-2 text-slate-300 hover:text-red-500 dark:text-slate-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100 print:hidden"
            aria-label="Xóa điểm"
          >
