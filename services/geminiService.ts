...
export const parseScoreFromText = async (
  ...
  try {
    ...
    if (customSubjects.length > 0) {
      subjectInstruction = `BẮT BUỘC: Tên môn học phải CHÍNH XÁC khớp với MỘT trong các môn sau (không được tạo môn mới): ${customSubjects.join(', ')}.
...
export const parseScoresFromImage = async (
  ...
  try {
    ...
    if (customSubjects.length > 0) {
      ...
      Chọn môn gần nghĩa nhất trong danh sách nếu không khớp chính xác`;
    }
